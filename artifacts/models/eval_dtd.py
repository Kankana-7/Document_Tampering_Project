...
def eval_net_dtd_with_threshold(model, test_data, thresholds=np.arange(0.1, 0.9, 0.1), device='cuda'):
    data_loader = DataLoader(dataset=test_data, batch_size=6, num_workers=4, shuffle=False)
    ckpt = torch.load(args.pth, map_location='cpu')
    model.load_state_dict(ckpt['state_dict'])
    model.eval()
    best_threshold = 0.0
    ...
    for threshold in thresholds:
        ...
        recalls = []
        with torch.no_grad():
            for batch_samples in tqdm(data_loader, desc=f"Threshold {threshold:.2f}"):
                ...
                logits = model(data, dct_coef, qs)
                probs = torch.softmax(logits, dim=1)[:, 1]
                preds = (probs > threshold).long()
                targ = target.squeeze(1)

                matched = (preds * targ).sum((1, 2)).float()
                pred_sum = preds.sum((1, 2)).float()
                targ_sum = targ.sum((1, 2)).float()

                precision = (matched / (pred_sum + 1e-8)).mean().item()
                recall = (matched / (targ_sum + 1e-8)).mean().item()
                f1 = (2 * precision * recall / (precision + recall + 1e-8))

                precisions.append(precision)
                recalls.append(recall)

                iou.add_batch(preds.cpu().numpy(), targ.cpu().numpy())

        acc, acc_cls, iu, mean_iu, fwavacc = iou.evaluate()
        avg_f1 = np.mean([2 * p * r / (p + r + 1e-8) for p, r in zip(precisions, recalls)])
        print(f"Threshold: {threshold:.2f} | mIoU: {mean_iu:.4f} | F1: {avg_f1:.4f}")
        if avg_f1 > best_f1:
            best_f1 = avg_f1
...
eval_net_dtd_with_threshold(model, test_data)
