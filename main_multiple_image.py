...
async def predict_tamper_region(files: List[UploadFile] = File(...)): # Accept a list of files
    try:
        start_time = time.time()
        
        # Create a dictionary to hold the results for all images
        aggregated_results = {}
        ...
        for file in files:
            ...
            # Add the result to our aggregated dictionary with the filename as the key
            aggregated_results[file.filename] = result

        processing_time = time.time() - start_time
        return JSONResponse(content={
            "status": "success",
            "result": aggregated_results, # Return the aggregated results
...
def upload_form():
    # Added the 'multiple' attribute to the file input and updated the script
    return """
    <html>
        <head>
        ...
        <body>
            <div class="container">
                ...
                <p>Please upload one or more image files to detect tampering...</p>
                <form action="/detect_tampering" enctype="multipart/form-data" method="post" style="display:flex; flex-direction:column; align-items:center;">
                    ...
                    <input id="file-upload" name="files" type="file" accept="image/*" onchange="showFileNames()" multiple>
            ...
            <script>
                function showFileNames() {
                    ...
                    if (input.files.length > 0) {
                        // Join all file names with a comma
...
if __name__ == "__main__":
    # Your uvicorn and ngrok setup remains the same
    ngrok.set_auth_token("2xzCCP6VkidD5aUYCnP2umNbOsw_5Smd8Er1QeQNopLhjnqrL") #<-- Make sure to use your token
    public_url = ngrok.connect(8000)
    print(f"🌐 Public URL: {public_url}")
    uvicorn.run("main_multiple_image:app", host="0.0.0.0", port=8000)